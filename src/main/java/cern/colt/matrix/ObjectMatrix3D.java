...
For an example, see {@link cern.colt.matrix.ObjectMatrix2D#getNonZeros(IntArrayList,IntArrayList,ObjectArrayList)}.
...
For an example see {@link cern.colt.matrix.ObjectMatrix2D#viewSelection(int[],int[])}.
<p>
...
For further information, see {@link cern.colt.matrix.objectalgo.Sorting#sort(cern.colt.matrix.ObjectMatrix3D,int,int)}.
